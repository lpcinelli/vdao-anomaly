...
VDAO_TEST_ENTRIES_OBJ_NB = [np.array([0, 2, 5, 6]), np.array([1, 3, 4, 8]), np.array([7, 9, 12, 13]),
                            np.array([10, 11, 14, 19]), np.array(
                                [15, 16, 20, 22]), np.array([17, 21, 24, 29]),
                            np.array([18, 28, 33, 38]), np.array(
                                [23, 25, 40, 41]), np.array([26, 27, 34, 39]),
                            np.array([30, 35, 36, 46]), np.array(
                                [31, 37, 43, 49]), np.array([32, 42, 44, 47]),
                            np.array([45, 50, 52, 56]), np.array(
                                [48, 51, 54, 55]), np.array([53, 56, 8, 10]),
...
AVGPOOL_SIZE = np.array(
    [21, 28, 28, 28, 21, 21, 21, 21, 14, 14, 14, 14, 14, 14, 7, 7, 7])
...
def changeModel(countList, cnt_comb, trn_shp):
    ...
    if (countList == 0):
        ...
        dense_model.add(Flatten(batch_input_shape=(
            None, trn_shp[1], trn_shp[2], trn_shp[3])))
...
def loadHDF5Batches(ctr_int, cnt_tst):
    ...
    print('LOADING TEST_SET /// ' +
          LAYER_NAME[ctr_int] + ' /// TEST_SET: ' + str(cnt_tst+1))

    for cnt_tmp in VDAO_TEST_ENTRIES_OBJ_NB[cnt_tst]:
        X_test_tmp = h5_file['.']['video' +
                                  str(cnt_tmp+1) + '_' + LAYER_NAME[ctr_int] + '_X_TEST_SET'].value
        y_test_tmp = h5_file['.']['video' +
                                  str(cnt_tmp+1) + '_' + LAYER_NAME[ctr_int] + '_y_TEST_SET'].value
        ...
        for obj_nb in range(VDAO_DATABASE_OBJECT_ENTRIES.shape[0]):
            for pos_nb in VDAO_DATABASE_OBJECT_POSITION_ENTRIES:
                if not obj_nb in tst_obj_lst:
                    ...
                    try:
                        X_train_tmp = h5_file['.'][ill_nb + '_' + VDAO_DATABASE_OBJECT_ENTRIES[obj_nb] +
                                                   '_' + pos_nb + '_' + LAYER_NAME[ctr_int] + '_X_TRAIN_SET'].value
                        y_train_tmp = h5_file['.'][ill_nb + '_' + VDAO_DATABASE_OBJECT_ENTRIES[obj_nb] +
                                                   '_' + pos_nb + '_' + LAYER_NAME[ctr_int] + '_y_TRAIN_SET'].value
            ...
            for cnt_trn in range(max_trn):
                ...
                if not (flag_0_HL):
                    ...
                    for eps in range(1, EPS_MAX_RNG+1):
                        ...
                        print('LAYER: ' + LAYER_NAME[ctr_int] + ' /// TEST_OBJECT: ' + str(cnt_tst+1) + ' /// HIDDEN_LYR_NB: ' + str(
                            countList) + ' /// TRAIN_NB: ' + str(cnt_dense_train) + ' /// EPOCH: ' + str(eps))

                        hist_fit = dense_model.fit(X_train, y_train, nb_epoch=(
                            1), batch_size=batch_max_val, shuffle=True, verbose=0)
                        ...
                        proba_vec_test = np.reshape(
                            proba_vec_test, (proba_vec_test.shape[0]))
                    ...
                    ##############################################################
						...
						if best_DISR_LOW < best_epoch_disr_low:
							best_epoch_disr_low = best_DISR_LOW
							best_epoch_low = eps
							proba_vec_low = proba_vec

						if best_DISR_HIGH < best_epoch_disr_high:
							best_epoch_disr_high = best_DISR_HIGH
							proba_vec_high = proba_vec
